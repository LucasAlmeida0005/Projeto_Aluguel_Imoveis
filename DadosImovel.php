...
<?php 
      //include('Login_v15/conexao.php');
      //require_once("Login_v15/painel.php");
      session_start();
      ...
      $sth = $dbh->prepare('SELECT id,nome FROM cadastro WHERE email = :email' );
...
<head>
	...
	<title>Dados Imóvel</title>
    <link rel="stylesheet" type="text/css" href="style.css">
	<link rel="stylesheet" type="text/css" href="DadosImovel.css">
...
<body>
  ...
  <header class="header">
    ...
    <!--<div class="InfoBloq inf">
            <h2>Informações com edição bloqueada.</h2>
            <p>Essas informações nas quais não está habilidado a possibilidade de edição de dados
            não podem serem alteradas devido ao fato de que os dados de identificação únicos, 
            assim impossibilitando a edição.</p>
        </div>-->
    ...
    ?>
     ...
     <div>
     <p class="info"><?php echo ucfirst(  $Resultado["nome"]);?>, essas são as informações de seus imóveis.</p>
     <hr class="hrheader">
     </div>
    ...
    <?php
          ...
          $sxh =  $db->prepare('SELECT * FROM cad_imovel WHERE iduser_fk = :iduser_fk');
          $sxh->bindParam(':iduser_fk', $fk, PDO::PARAM_STR);
          $sxh->execute();
          $dadosimovel = $sxh->fetchAll(PDO::FETCH_ASSOC);

      if(count($dadosimovel)){
    ...
    <form class="lista">
        ...
        <div>
            <h3>Valor</h3>
            <p><?php  echo $Dados["valor"]?></p>
            <a href="editarimoveis.php?id=<?php echo $Dados['id'] ?>">Editar</a>
        </div> 
        ...
        <div>
         ...
         <div>
            ...
            <p><?php  echo $Dados["tipoimovel"]?></p>
              <a href="editarimoveis.php?id=<?php echo $Dados['id'] ?>">Editar</a>
        </div>

         <div>
            <h3>Metros quadrados</h3>
            <p><?php  echo $Dados["metroquadrado"]?>M²</p>
             <a href="editarimoveis.php?id=<?php echo $Dados['id'] ?>">Editar</a>
        </div>
        <div>
            <h3>Quantos comodos</h3>
            <p><?php  echo $Dados["qtdcomodos"]?></p>
             <a href="editarimoveis.php?id=<?php echo $Dados['id'] ?>">Editar</a>
        </div>
        <div>
            <h3>Mobiliado</h3>
            <p><?php  echo $Dados["mobilia"]?></p>
             <a href="editarimoveis.php?id=<?php echo $Dados['id'] ?>">Editar</a>
        </div>
        
            <a href="delete.php?id=<?php echo $Dados['id'] ?>" class="delete">Deletar</a>
       

    </form>

    <form class="fotoimovel">
       <div>

           <?php  echo   "<img src='upload/" .$Dados['arquivo'] ."' width='100%' height='100%' ' >"?>
    ...
    <center><h1 style="font-family: arial; padding-top: 30px;">Nenhum imóvel registrado!</h1></center>
